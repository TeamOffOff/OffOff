...
class PostListControl(Resource):
    def get(self, page_size=5, last_id=None):  #이 파라미터는 어떻게 전달되는걸까?
        ...
        if last_id is None:
            ...
            cursor = list(mongodb.find(collection_name="board_type_post").sort([("_id", -1)]).limit(page_size))
            #최신순으로 정렬해서(내림차순) 그냥 갯수만큼만 불러오면 됨
        else: 
            #사용자가 스크롤을 끝까지 내려서 새로운 리스트를 받아오는 경우
            cursor = list(mongodb.find(query={'_id': {'$lt': self.last_id} }, collection_name="board_type_post").sort([("_id", -1)]).limit(page_size))
        ...
        last_id = str(cursor[-1]['_id'])
        #해당 리스트의 제일 마지막 요소의 _id값을 받아와야함


        return {
            "post_list" : json.dumps(cursor, default=json_util.default), 
            "last_id" : last_id
        }
