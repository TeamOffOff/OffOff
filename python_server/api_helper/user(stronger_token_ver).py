...
class AuthRegister(Resource):
    ...
    def post(self):  # 회원가입
        ...
        access_token_encoded = jwt.encode(access_payload, SECRET_KEY, ALGORITHM)  
        
        refresh_token_encoded = jwt.encode(refresh_payload, SECRET_KEY, ALGORITHM)
        

        if not (type(access_token_encoded) is str):  # 버전에 따라 token의 자료형이 다름(string, byte) 따라서 type casting 필요
            access_token_encoded = access_token_encoded.decode("UTF-8")
        
        if not (type(refresh_token_encoded) is str):
            refresh_token_encoded = refresh_token_encoded.decode("UTF-8")
        ...
        try: 
            ...
            for key in sub_info_key:
                real_user_sub_information[key]=user_info["subInformation"][key]
            
            activity_key = ["posts", "replies", "likes", "reports", "bookmarks"]
            real_user_activity = {}
            for key in activity_key:
                real_user_activity[key] = user_info["activity"][key]
            ...
            real_user_info["refreshToken"] = refresh_token_encoded
            ...
            return {
                'Authorization': access_token_encoded,
                'refreshToken': refresh_token_encoded,
                "queryStatus": 'success'
            }, 200
...
class AuthLogin(Resource):
    ...
    def get(self):  # 회원정보조회
        ...
        try:
            ...
            access_token_decoded = jwt.decode(header, SECRET_KEY, ALGORITHM)  # {'id':실제 id} 딕셔너리형태로 돌려줌
            user_info = mongodb.find_one(query={"_id": access_token_decoded["_id"]}, collection_name="user")
            
            # 회원정보 보내줄 때 비밀번호는 안 보이게, refresh 토큰은 없애고 보내줘야함
