...
constructor(private val boardService: BoardService) {
    ...
    suspend fun getNextPosts(boardType: String, lastPostId: String) =
        boardService.getNextPosts(boardType, lastPostId)
    ...
    suspend fun doPostActivity(auth: String, activityItem: ActivityItem) =
        boardService.doPostActivity(auth, activityItem)

    suspend fun getComments(auth: String, postId: String, boardType: String) =
        boardService.getComments(auth, postId, boardType)

    suspend fun writeComment(auth: String, comment: CommentSend) =
        boardService.writeComment(auth, comment)

    suspend fun deleteComment(auth: String, comment: CommentSend) =
        boardService.deleteComment(auth, comment)

    suspend fun likeComment(auth: String, activityItem: ActivityItem) =
        boardService.likeComment(auth, activityItem)
